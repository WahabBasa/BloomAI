...
GENERIC_ERROR = 'An internal error occurred. Please try again.'
...
def upload_document(request):
    ...
    # Prefix with a UUID so two users uploading "chapter1.pdf" don't overwrite
    # each other, and so a crafted filename can't collide with an existing file.
    ...
    try:
        ...
        logger.exception('Failed to write uploaded file to %s', full_path)
        return JsonResponse({'error': GENERIC_ERROR}, status=500)

    try:
        # Creating the document and generating its questions is one unit of
        # work: if generation fails we must not leave an empty document row
        # (and its now-orphaned file) behind.
        ...
        logger.exception('Failed to process uploaded document %s', file_path)
        _remove_quietly(full_path)
        return JsonResponse({'error': 'Failed to process the uploaded document.'}, status=500)


def _remove_quietly(path):
    """Best-effort cleanup of an orphaned upload."""
    try:
        os.remove(path)
    except OSError:
...
def get_document(request, document_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # get_object_or_404 signals a missing row; let Django render the 404
            raise
        except Exception:
...
def get_questions(request, document_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            logger.exception('Failed to fetch questions for document %s', document_id)
            return JsonResponse({'error': GENERIC_ERROR}, status=500)
...
def submit_answer(request, question_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            logger.exception('Failed to grade answer for question %s', question_id)
            return JsonResponse({'error': GENERIC_ERROR}, status=500)
...
def get_answer(request, answer_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            logger.exception('Failed to fetch answer %s', answer_id)
            return JsonResponse({'error': GENERIC_ERROR}, status=500)
